...
def _score_1d_candidates(bin_frame: pd.DataFrame, labels: pd.DataFrame) -> pd.DataFrame:
    ...
    for feature in bin_frame.columns:
        ...
        for label in labels.columns:
            frame = (
                pd.DataFrame({"bin": feature_bins, "label": labels[label]})
                .loc[valid_feature]
                .dropna()
            )
    ...
    return (
        pd.DataFrame(rows)
        .sort_values(["score", "samples"], ascending=[False, False])
...
def _select_candidate_features(
    ...
    if remaining and len(selected) < max_features:
        ...
        sampled = list(
            rng.choice(np.array(remaining, dtype=object), size=sample_size, replace=False)
        )
        selected.extend(str(feature) for feature in sampled)
    return selected[:max_features]


def _evaluate_2d_grid_purity(
    df: pd.DataFrame, x_bin_col: str, y_bin_col: str, x_val_col: str, y_val_col: str, label_col: str
) -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame()
...
def _merge_contiguous_regions(
    ...
    if grid_cells is None or grid_cells.empty:
        return pd.DataFrame()
    filtered = grid_cells[
        (grid_cells["purity"] >= min_purity) & (grid_cells["sample_count"] >= min_samples)
    ].copy()
    if filtered.empty:
        return pd.DataFrame()
...
def _extract_2d_rules(
    ...
    for fx, fy in combinations(candidates, 2):
        ...
        merged_regions = _merge_contiguous_regions(
            cells, None, fx, fy, label_col, min_purity, min_samples
        )
...
def _render_rules_markdown(
    metadata: VisualRegionsRunMetadata, rules_df: pd.DataFrame, candidate_scores: pd.DataFrame
) -> str:
    lines = [
    ...
    if rules_df.empty:
        ...
        for _, rule in rules_df.head(10).iterrows():
            lines.append(
                f"- **Rank {rule['rank']}**: {rule['rule_text']} (Purity: {rule['purity_pct']:.2%}, Samples: {rule['sample_count']})"
            )
        lines.append("")
...
def _render_rules_html(
    markdown: str, rules_df: pd.DataFrame, candidate_scores: pd.DataFrame
) -> str:
...
class VisualRegionsModule:
    ...
    def run(
        ...
        with ModuleProgress(self.name, total=5, enabled=self.progress) as progress_bar:
            ...
            progress_bar.step("extract_rules")
            rules_df = _extract_2d_rules(
                merged,
                feature_cols,
                label_cols,
                n_bins=self.n_bins,
                min_samples=self.min_samples_per_region,
