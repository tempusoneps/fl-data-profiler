...
def load_template(skill_dir: Path, kind: str = "insights") -> dict[str, Any]:
    asset_name = "insights-template.ipynb"
...
def update_title(notebook: dict[str, Any], kind: str, title: str) -> None:
    if title.startswith("#"):
        expected = f"{title}\n"
    elif kind in ("insights", "insight"):
        expected = f"# Insights: {title}\n"
    elif kind == "analytics":
        expected = f"# Analytics: {title}\n"
    elif kind == "tutorial":
        expected = f"# Tutorial: {title}\n"
    else:
        expected = f"# {title}\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scaffold a Jupyter notebook for quantitative insights and analytics.")
    parser.add_argument(
        "--kind",
        choices=["insights", "analytics", "tutorial"],
        default="insights",
        help="Notebook style to scaffold (default: insights).",
    )
    parser.add_argument(
        "--title",
        required=True,
